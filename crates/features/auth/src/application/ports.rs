...
pub trait AuthRepository: Send + Sync {
    ...
    async fn find_user_by_oauth(&self, provider: &str, provider_account_id: &str) -> anyhow::Result<Option<(UserId, String)>>;
    async fn upsert_oauth_user(&self, provider: &str, provider_account_id: &str, email: Option<&str>, handle: &str) -> anyhow::Result<UserId>;
