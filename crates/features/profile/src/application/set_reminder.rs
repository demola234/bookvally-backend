...
impl<R: ProfileRepository> UpdateReminder<R> {
    pub async fn execute(&self, user_id: Uuid, input: UpdateReminderInput) -> Result<(), AppError> {
        ...
        if let Some(t) = input.time_local {
            reminder.time_local = t;
        }
        if let Some(d) = input.days_of_week {
            reminder.days_of_week = DaysOfWeek::from_bits(d);
        }
