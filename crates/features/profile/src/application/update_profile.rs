...
impl<R: ProfileRepository> UpdateProfile<R> {
    pub async fn execute(
        ...
        if let Some(bio) = input.bio {
            profile.update_bio(Some(bio));
        }
        if let Some(p) = input.pronouns {
            profile.update_pronouns(Some(p));
        }
        if let Some(l) = input.location {
            profile.update_location(Some(l));
        }
        if let Some(b) = input.banner_url {
            profile.update_banner_url(Some(b));
        }
        if let Some(g) = input.favorite_genres {
            profile.update_favorite_genres(Some(g));
        }
        ...
        if let Some(s) = input.reading_since {
            let date = NaiveDate::parse_from_str(&s, "%Y-%m-%d").map_err(|_| {
                AppError::UnprocessableEntity("invalid reading_since format, use YYYY-MM-DD".into())
            })?;
