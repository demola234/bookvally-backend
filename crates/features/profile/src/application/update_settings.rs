...
impl<R: ProfileRepository> UpdateSettings<R> {
    pub async fn execute(
        ...
        let updated = UserSettings {
            ...
            daily_goal_minutes: input
                .daily_goal_minutes
                .unwrap_or(existing.daily_goal_minutes),
            activity_sharing: input.activity_sharing.unwrap_or(existing.activity_sharing),
            contact_matching_opt_in: input
                .contact_matching_opt_in
                .unwrap_or(existing.contact_matching_opt_in),
            updated_at: Utc::now(),
        };

        self.repository
            .upsert_settings(&updated)
            .await
            .map_err(AppError::internal)?;
