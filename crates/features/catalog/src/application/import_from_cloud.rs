...
impl<R: CatalogRepository> ImportFromCloud<R> {
    pub async fn execute(&self, user_id: Uuid, input: ImportFromCloudInput) -> Result<Uuid, AppError> {
        let format = detect_format(&input.file_name)
            .ok_or_else(|| AppError::UnprocessableEntity(
                "unsupported file format — only .pdf and .epub are accepted".into()
            ))?;

        let imported = self.importer
...
fn detect_format(file_name: &str) -> Option<BookFormat> {
    ...
    if lower.ends_with(".epub") { return Some(BookFormat::Epub); }
    if lower.ends_with(".pdf")  { return Some(BookFormat::Pdf); }
    None
}
