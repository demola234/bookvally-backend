        ...
        let book_id = match file.book_id {
            Some(id) => {
                    ...
                    .map_err(AppError::internal)?
                {
                    ...
                    if let Some(p) = parsed.page_count {
                        book.total_pages = Some(p);
                    }
                    if let Some(l) = parsed.language {
                        book.metadata_source = Some(l);
                    }
                    self.repository
                        .update_book(&book)
                        .await
            ...
            None => {
                let mut book = Book::new(parsed.title.unwrap_or_else(|| file.file_name.clone()));
